...
#     print(f"Upserting tags:\nTotal: {len(specialties) + len(r_terms) + len(topics)}. ({len(specialties)} specialties, {len(r_terms)} related terms, {len(topics)} topics)...")
#     try:
#         # 1. Upsert all unique items in one go
#         s_data = [{'name': name, 'type': 'SPECIALTY'} for name in specialties]
#         r_data = [{'name': name, 'type': 'RELATED_TERM'} for name in r_terms]
#         t_data = [{'name': name, 'type': 'TOPIC'} for name in r_terms]
        
#         r_response = await supabase.table('tags').upsert(
#             r_data, on_conflict='name'
#         ).execute()
#         t_response = await supabase.table('tags').upsert(
#             t_data, on_conflict='name'
#         ).execute()
#         s_response = await supabase.table('tags').upsert(
#             s_data, on_conflict='name'
#         ).execute()
...
async def batch_upload_questions(all_questions: list, supabase: AsyncClient):
    ...
    # === STEP 1: Pre-process all Specialties and Related Terms ===
    # This is *massively* more efficient. We find all unique terms
    # across all 20,000 questions and upload them in two API calls.
    
    print("Pre-processing: Finding all unique specialties and terms...")

    # === STEP 2: Upload unique terms/specialties and get their IDs ===
    all_tags = []  
    with open('all_tags.json', 'r', encoding='utf-8') as f:
        all_tags = json.load(f)

    all_tags_map = {item['name']: item['id'] for item in all_tags}
    ...
    try:
        for i in range(0, len(all_questions)):
            ...
            q_res = await supabase.table("questions").insert(question_to_insert).execute()
            if not q_res.data:
                failed += 1
                print(f"Failed to insert question {i+1}. Skipping...")
                print(f'ailed questions so far: {failed}')
                continue        
            ...
            q_tags.extend(q_data.get("specialty", []))
            q_tags.extend(q_data.get("related_terms", []))
            q_tags.extend(q_data.get("related_topics", []))
            q_tags_links = [{'question_id': new_question_id, 'tag_id': all_tags_map.get(tag_name)} for tag_name in q_tags if all_tags_map.get(tag_name)]

            await supabase.table("question_tags").insert(q_tags_links).execute()
            successful += 1
            print(f'Successfully inserted question {i+1}. Total successful: {successful}')
    except APIError as e:
...
async def main():
    ...
    for i in range(2, 7):
        print(f"Loading questions from file output_questions_{i}.json...")
        all_questions = load_questions_from_file(f"output_questions_{i}.json")
